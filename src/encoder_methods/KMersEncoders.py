...
from encoder_methods.encoders import Encoders


class KMersEncoders(Encoders):
    def __init__(self, dataset=None, sequence_column=None, size_kmer=3, ignore_columns=[]) -> None:
        super().__init__(
            dataset=dataset, sequence_column=sequence_column, ignore_columns=ignore_columns
        )
    ...
    def process_dataset(self):
        ...
        processed_data = tfidfvector.fit_transform(self.dataset["kmer_sequence"]).astype("float32")

        self.coded_dataset = pd.DataFrame(
            data=processed_data.toarray(), columns=tfidfvector.get_feature_names_out()
        )
