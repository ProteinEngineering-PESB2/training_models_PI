...
class ESMBasedEmbedding(EmbeddingBased):
            ...
            for sequence in batch:
                try:
                    ...
                    embedding_config = LogitsConfig(
                        return_embeddings=True, return_hidden_states=True
                    )
                    logits_output = name_model.logits(protein_tensor, embedding_config)

                    last_layer_embedings_mean = torch.mean(
                        logits_output.embeddings, dim=1
                    ).squeeze()
    ...
    def embedding_process(
        self, seq_len: Optional[int] = None, batch_size: int = 100
    ) -> Tuple[List, List]:
        ...
        for i in tqdm(
            range(0, len(padded_sequences), batch_size), desc="[+] Embedding: ", unit="batch"
        ):
            ...
            states_means.extend(
                [torch.mean(result["hidden_states"], dim=-2).squeeze() for result in batch_results]
            )
        ...
        for layer_idx in range(n_layers):
            layers_embeddings.append(
                torch.stack([layer[layer_idx, :] for layer in states_means]).float().cpu().numpy()
            )
    ...
    def export_layers_processed(
        self, layers_embeddings: List[np.ndarray], path, prefix: str
    ) -> None:
